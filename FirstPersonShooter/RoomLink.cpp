...
RoomLink RoomLink::MakeRoomLink(Vector3 pos, Vector3 size)
{
    ...
    Vector3 linkPos = Vector3(
        pos.x + (size.x > 0 ? RngUtils::RandIntInRange(0, size.x - 1) : 0),
        pos.y + (size.y > 0 ? RngUtils::RandIntInRange(0, size.y - 1) : 0),
        pos.z + (size.z > 0 ? RngUtils::RandIntInRange(0, size.z - 1) : 0)
    );
