...
{
    if (GeometryUtils::LinesIntersect1D(pos.x, pos.x + size.x, room.pos.x, room.pos.x + room.size.x)
        && GeometryUtils::LinesIntersect1D(pos.y, pos.y + size.y, room.pos.y, room.pos.y + room.size.y)
        && GeometryUtils::LinesIntersect1D(pos.z, pos.z + size.z, room.pos.z, room.pos.z + room.size.z))
    {
        // Check if rooms are not adjacent by diagonal (only one must be true)
        return GeometryUtils::LinesIntersectByEnds1D(pos.x, pos.x + size.x, room.pos.x, room.pos.x + room.size.x)
            + GeometryUtils::LinesIntersectByEnds1D(pos.y, pos.y + size.y, room.pos.y, room.pos.y + room.size.y)
            + GeometryUtils::LinesIntersectByEnds1D(pos.z, pos.z + size.z, room.pos.z, room.pos.z + room.size.z)
            == 1;
    }
    else
        return false;
...
std::tuple<Vector3, Vector3> GeneratedRoom::ComputeBorders(GeneratedRoom room)
{
    ...
    // Borders in axis YZ
    if (GeometryUtils::LinesIntersectByEnds1D(pos.x, pos.x + size.x, room.pos.x, room.pos.x + room.size.x))
    {
        int xPos = (pos.x == room.pos.x + room.size.x) ? pos.x : pos.x + size.x;
        auto yInt = GeometryUtils::ComputeLinesIntersection1D(pos.y, pos.y + size.y, room.pos.y, room.pos.y + room.size.y);
        auto zInt = GeometryUtils::ComputeLinesIntersection1D(pos.z, pos.z + size.z, room.pos.z, room.pos.z + room.size.z);

        borderPos = Vector3(xPos, std::get<0>(yInt), std::get<0>(zInt));
        borderSize = Vector3(0, std::get<1>(yInt), std::get<1>(zInt));
    }
    // Borders in axis XZ
    if (GeometryUtils::LinesIntersectByEnds1D(pos.y, pos.y + size.y, room.pos.y, room.pos.y + room.size.y))
    {
        int yPos = (pos.y == room.pos.y + room.size.y) ? pos.y : pos.y + size.y;
        auto xInt = GeometryUtils::ComputeLinesIntersection1D(pos.x, pos.x + size.x, room.pos.x, room.pos.x + room.size.x);
        auto zInt = GeometryUtils::ComputeLinesIntersection1D(pos.z, pos.z + size.z, room.pos.z, room.pos.z + room.size.z);
    ...
    // Borders in axis XY
    if (GeometryUtils::LinesIntersectByEnds1D(pos.z, pos.z + size.z, room.pos.z, room.pos.z + room.size.z))
    {
